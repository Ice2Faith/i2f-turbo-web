import {n as p} from "./chunk-ENMKPL7Y.mjs";
import {_ as s, j as m} from "./chunk-KEUXMURM.mjs";
import {a as o} from "./chunk-AQ6EADP3.mjs";

var d = o(({flowchart: n}) => {
    let i = n?.subGraphTitleMargin?.top ?? 0, e = n?.subGraphTitleMargin?.bottom ?? 0, r = i + e;
    ...
    await Promise.all([...e].map(t => new Promise(g => {
        function a() {
            if (t.style.display = "flex", t.style.flexDirection = "column", r) {
                let c = s().fontSize ? s().fontSize : window.getComputedStyle(document.body).fontSize,
                    u = 5, [f = m.fontSize] = p(c), l = f * u + "px";
                t.style.minWidth = l, t.style.maxWidth = l
            } else t.style.width = "100%";
            g(t)
        }

        o(a, "setupImage"), setTimeout(() => {
            t.complete && a()
        }), t.addEventListener("error", a), t.addEventListener("load", a)
    })))
}

o(G, "configureLabelImages");
